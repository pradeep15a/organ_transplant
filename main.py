...
def add_User():
    ...
    for field1 in fieldss:
        temp1 = request.form.get(field1)
        if temp1 != '':
            temp1 = "\'"+temp1+"\'"
        if temp1 == '':
            temp1 = 'NULL'
    ...
    print(qry)
    qry2 = "INSERT INTO Login Values (%s,%s)"%logval
    print(qry2)
    success = True
    error = False
    try: 
    ...
    return redirect(url_for('home', id='User', error=error,success=success))
...
def add_Organization():
    ...
    for field in fields:
        temp = request.form.get(field)
        if field not in ['Organization_ID'] and temp != '':
            temp = "\'"+temp+"\'"
        if temp == '':
            temp = 'NULL'
        val = val + (temp,)
...
def stats():
    ...
    for organ in organ_list:
        qry = "select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where Organ_donated = '%s' and Status = 'Success'" %organ
        print(qry)
        mycursor.execute(qry)
        a = mycursor.fetchone()
        A.append(a[0])
        qry = "select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where Organ_donated = '%s' and Status = 'Failure'" %organ
        print(qry)
        mycursor.execute(qry)
        b = mycursor.fetchone()
        B.append(b[0])
